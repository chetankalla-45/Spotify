...
const AddSong = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try{
            ...
            const responce = await axios.post(`${url}/api/song/add`,formData);
    ...
    const loadAlbumData = async () => {
        try{
            ...
            if(responce.data.success){
                setAlbumData(responce.data.albums);
            }
            else{
  ...
  return loading ? (
    ...
    <form onSubmit={onSubmitHandler} action="" className='flex flex-col items-start gap-8 text-gray-800'>
        ...
        <div className='flex flex-col gap-2.5'>
            <p>Album</p>
            <select onChange={(e)=>setAlbum(e.target.value)} defaultValue={album} className='bg-transparent outline-gray-600 border-2 border-gray-400 p-2.5 w-[150px]'>
